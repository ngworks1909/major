...
export default function Docs() {
  ...
  const handleDelete = async(id) => {
    const response = await fetch(`http://localhost:3001/api/operation/deleteFile/${id}`, {
              method: "DELETE",
              headers: {
                  "Content-Type": "application/json",
                  "auth-token": authToken
    ...
    const json = await response.json();
    if(json.success){
      const newDocs = docs.filter((document)=>{return document.fileId !== id});
      setDocs(newDocs);
            ...
            <table className="tbl">
              ...
              <tbody>
                {filteredDocs.length > 0 ? filteredDocs.map((doc, index) => (
                  <tr key={`docs${doc.fileId}`}>
                    <td>{index + 1}</td>
                    <td autoCapitalize='false'>{doc.filename}</td>
                    <td>{dateconvert(doc.createdAt)}</td>
                    <td><a href={doc.url} rel="noreferrer" className="clickb" target="_blank">Click Here </a></td>
